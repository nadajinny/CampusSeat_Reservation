...
`frontend/__tests__/reservation-engine.test.js`에서 유틸리티 메서드도 직접 검증합니다.

| 케이스 | 목적 |
| --- | --- |
| 슬롯 겹침 경계 | 끝-시작이 맞닿는 슬롯은 겹치지 않는지 확인 |
| 슬롯 겹침 감지 | 부분 겹침이 true로 판정되는지 확인 |
| 리스트 겹침 감지 | 슬롯 리스트 내 중복 여부 확인 |
| 날짜 파싱 | 잘못된 날짜 문자열이 null로 처리되는지 확인 |
| 과거 날짜 판정 | 잘못된 날짜 입력이 false를 반환하는지 확인 |
| 주간 범위 | 월요일 시작/일요일 끝 범위를 반환하는지 확인 |
| 주간 합산 | 같은 주에 속한 회의실 예약만 합산되는지 확인 |
| 좌석 가용성 | 좌석/슬롯 누락 시 false 반환 및 겹침 예약 거부 확인 |
| 사용자 충돌 | 동일 사용자의 예약 겹침을 반환하는지 확인 |
| 회의실 선점 | 동일 회의실/슬롯 선점 여부 확인 |

---

## 4. ApiClient 테스트 커버리지

`frontend/js/api-client.js`는 세션 인증과 API 요청 흐름을 담당하며, `frontend/__tests__/api-client.test.js`에서 다음 항목을 검증합니다.

| 케이스 | 목적 |
| --- | --- |
| 인증 상태 관리 | `getAuth`, `setAuth`, `clearAuth` 동작 확인 |
| 헤더 자동 부착 | JSON 요청 시 `Content-Type`, 토큰 존재 시 `Authorization` 확인 |
| 응답 파싱 | JSON/텍스트 응답 처리 확인 |
| 오류 처리 | `response.ok` 실패 및 `is_success === false` 처리 확인 |
| 로그인 플로우 | 학번 정규화 및 토큰 저장 확인 |
| 예약 생성 | 회의실/좌석 예약 경로 및 payload 직렬화 확인 |
| 예약 조회 | 쿼리 파라미터 직렬화 규칙 확인 |
| 예약 취소 | DELETE 메서드와 경로 확인 |
| 상태 조회 | 회의실/좌석 상태 조회 경로 확인 |
